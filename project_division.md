...
**Owns:** Features 1, 2, 3, 4, 5, 6  
**Role:** Foundation pipeline — everything downstream depends on Zain's output.
...
**Owns:** Features 7, 11, and pipeline integration  
**Role:** Ball intelligence + final assembly.

Lamaan does not need the SportsMOT dataset. Ball detection development and testing can be done on any football clip from YouTube — the model runs on single images. Touch detection logic is pure geometry and can be tested with mock coordinate inputs. Pipeline integration is naturally Lamaan's because his features require both Zain's tracking output and Sameem's team labels — he is the one connecting everything together at the end.
